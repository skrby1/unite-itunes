...
function! s:uifilter.filter(candidates, context)
  ...
  for candidate in a:candidates
    ...
    let candidate.word = printf(format, -1 * (float2nr(winwidth(0) * 0.845)), pl.name, pl.dur)
  endfor
  return a:candidates
endfunction
...
function! s:unite_itunes.action_table.enter_track.func(candidate)
  exe "Unite -buffer-name=itunes_tracks itunes_tracks:". escape(a:candidate.action__play_name, ' :')
endfunction

function! s:unite_itunes.hooks.on_init(args, context)
  call system('osascript '. expand("~/.vim/bundle/unite-itunes/autoload/unite/ps_check.applescript"))
  if len(a:args) != 0
    if a:args[0] == '!'
...
function! s:unite_itunes.gather_candidates(args, context) abort
  let s:songs = []
  let l:sys = 'osascript '. expand("~/.vim/bundle/unite-itunes/autoload/unite/for_unite.applescript"). ' | perl -pe "s/\r/\n/g"'
